...
## @class RflyMavrosStart
# @brief RflyMavrosStart结构体类.   
# @details
class RflyMavrosStart:
    ...
    # @brief RflyMavrosStart的构造函数
    # @param 初始化参数为：飞机的ID号、飞机的ip地址、飞机与Pixhawk的连接模式、端口号、飞机模型
    # @return 无
    def __init__(self, ID=1, ip='127.0.0.1', namespace='', Com='udp', port=0, simulinkDLL=False, launch_file=None, config_file=None, plugin_file=None):
        self.isCom = False
        self.Com = Com
        self.baud = 115200
        self.isRealFly = 0
    ...
    def InitRosLoop(self):
        ...
        namespace_str = ''
        if self.namespace != '':
            namespace_str = f" namespace:={self.namespace}/mavros"
        launch_file_str = 'mavros px4.launch'
        if not is_use_ros1:
            launch_file_str = 'mavros px4.launch.xml'
        if self.launch_file and os.path.isfile(self.launch_file):
            launch_file_str = os.path.abspath(self.launch_file)
        config_str = ''
        if self.config_file and os.path.isfile(self.config_file):
            yaml_file_path = os.path.abspath(self.config_file)
            config_str = f'config_yaml:={yaml_file_path} '
        plugin_str = ''
        if self.pulgin_file and os.path.isfile(self.pulgin_file):
            yaml_file_path = os.path.abspath(self.pulgin_file)
            plugin_str = f'pluginlists_yaml:={yaml_file_path} '
        ...
        if self.isCom:
            ...
            print(f"Target System: {self.tgtSys}")
            # 然后用mavros连接
            if is_use_ros1:
                cmdStr = f'roslaunch {launch_file_str} tgt_system:=' + \
                    str(self.tgtSys)+' fcu_url:="serial://'+self.ComName + \
                    ':'+str(self.baud)+'"'+namespace_str + \
                    ' '+config_str+' '+plugin_str
            else:#
                cmdStr = f'ros2 launch {launch_file_str} tgt_system:=' + \
                    str(self.tgtSys)+' fcu_url:="serial://'+self.ComName + \
                    ':'+str(self.baud)+'"'+namespace_str + \
                    ' '+config_str+' '+plugin_str
        else:
            ...
            # 然后用mavros连接
            if is_use_ros1:
                cmdStr = f'roslaunch {launch_file_str} tgt_system:='+str(self.tgtSys)+' fcu_url:="udp://:'+str(int(self.port)+1)+'@'+self.ip+':'+str(self.port)+'"'+namespace_str+' '+config_str+' '+plugin_str
            else:
                cmdStr = f'ros2 launch {launch_file_str} tgt_system:='+str(self.tgtSys)+' fcu_url:="udp://:'+str(int(self.port)+1)+'@'+self.ip+':'+str(self.port)+'"'+namespace_str+' '+config_str+' '+plugin_str
